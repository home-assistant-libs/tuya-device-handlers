...
def test_quirk_overrides(
    ...
    # 10x too high, and reports the energy unit as `度` (Chinese for kWh).
    definitions = get_sensor_default_definitions(device)
    assert definitions["cur_voltage"].sensor_wrapper.read_device_status(
        device
    ) == (2306)
    assert definitions["cur_power"].sensor_wrapper.read_device_status(
        device
    ) == (2941)
